...
document.querySelectorAll('.tit_card').forEach((card)=>{
  ...
  card.addEventListener('mousemove',(e)=>{
    ...
    const py = (e.clientX - rect.top) / rect.height;
    //위아래 위치를 rotateX 각도로 바꿀 것
    const rx = (py - 0.5) * -15;
    const ry = (px - 0.5) * 22;

    //계산한 각도를 안쪽 레이어 transform에 적용
    layer.style.transform = `rotateZ(0) rotateX(0deg) rotate(0deg)`;
...
const processCard = gsap.untils.toArray('.projects_all .card');

processCards.forEach((card, index) => {
  //카드 순서에 맞는 이미지 파일 경로를 만든다
  const imgSrc = `asset/info${index+1}.png`;
...
//sticky box의 이미지를 바로 바꾸면 딱딱하게 바껴서 살짝 줄였다가 다시 키우며 교체
...
//horizontal_section은 세로 스크롤을 가로 이동으로 바꿔서 보여줍니다.
const horizontalTween = gsap.to('.track',{
  //트랙을 왼쪽으로 이동시켜 오른쪽 카드들이 보이게 할 것
  x: () => -totalWidth(),
  ease : 'none',
  scrollTrigger:{
...
window.addEventListener('load',()=>{
  ...
  if(document.fonts){
    //폰트가 모두 준비된 뒤에 스크롤 트리거 위치를 다시 맞춘다.
